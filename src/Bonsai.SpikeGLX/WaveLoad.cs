...
namespace Bonsai.SpikeGLX
{
    ...
    [TypeConverter(typeof(WaveLoadConverter))]
    [Description("TO-DO.")]
        ...
        public IObservable<T> Process<T>(IObservable<T> source)
        {
            return Observable.Using(() => new SpikeGLX(Host, Port),
                connection =>
                {
                    ...
                    {
                        case DeviceType.Daq:
                            return source.Do(input => connection.NIWaveLoad(OutChan, Wave, Loop));
                        case DeviceType.Onebox:
                            return source.Do(input => connection.ObxWaveLoad(Ip, Slot, Wave));
                        default:
                            return source;
                    }
                });
    ...
    public class WaveLoadConverter : ExpandableObjectConverter
    {
